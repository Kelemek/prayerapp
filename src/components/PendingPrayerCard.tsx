...
interface PendingPrayerCardProps {
  ...
  onEdit: (id: string, updates: { title?: string; description?: string; requester?: string; prayer_for?: string; email?: string | null }) => Promise<void>;
}

export const PendingPrayerCard: React.FC<PendingPrayerCardProps> = ({ 
  prayer, 
  onApprove, 
  onDeny,
  onEdit
  ...
  const handleDeny = async (e: React.FormEvent) => {
    ...
    setLoading('deny');
    try {
      await onDeny(prayer.id, denyReason);
      setDenyReason('');
          ...
          {isEditing ? (
            ...
            <>
              ...
              <div className="space-y-2 text-sm text-gray-500 dark:text-gray-400 mb-4">
                ...
                <div className="flex items-center gap-1">
                  <Calendar size={14} />
                  <span>Requested {formatDate(prayer.date_requested)}</span>
                </div>
                <div className="flex items-center gap-1">
                  <CheckCircle size={14} />
      ...
      {showDenyForm && (
        <form onSubmit={handleDeny} className="mt-4 p-4 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-800">
          ...
          <textarea
            ...
            placeholder="Explain why this prayer request cannot be approved..."
