...
export const PendingDeletionCard: React.FC<PendingDeletionCardProps> = ({
  ...
  useEffect(() => {
    const lookupEmail = async () => {
      if (!deletionRequest.requested_email) return;

      setPcLoading(true);
      setPcError(false);
      
      try {
        const result = await lookupPersonByEmail(deletionRequest.requested_email);
    ...
    <>
      ...
      {showDenyForm && (
        <form onSubmit={handleDeny} className="w-full mt-4 p-4 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-800">
          ...
          <textarea id={`denial-reason-${deletionRequest.id}`} value={denialReason} onChange={(e) => setDenialReason(e.target.value)} className="w-full px-3 py-2 text-sm border border-red-300 dark:border-red-600 rounded-md bg-white dark:bg-red-900/30 text-gray-900 dark:text-red-100 placeholder-red-400 focus:outline-none focus:ring-2 focus:ring-red-500 resize-none" rows={3} placeholder="Explain why this deletion request is being denied..." required />
