...
function tryResolveImport(baseDir, spec) {
  // Try resolving from baseDir and also try climbing up directories in case spec
  // was written relative to a parent of the test file.
  const maxUp = 4
...
function processFile(filePath) {
  ...
  if (filePath.includes(`${path.sep}__tests__${path.sep}`)) return
...
console.log('Component test move complete')
